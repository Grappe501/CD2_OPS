...
export async function POST(req: NextRequest) {
  ...
  const prompt = AI_PROMPTS.fr_followup_draft;
  ...
  const requires = requireApproval();
  const status = requires ? "generated" : "approved";
  ...
  const row = await sql.one(`
    ...
    VALUES
      ($1,$2,$3,$4,$5,false,$6,$7,$8,'prospect',$9,$10)
    RETURNING suggestion_id, status, created_at
  `, ["fr_followup_draft", prompt.version, modelPrimary(), JSON.stringify(inputs), text, requires, status, actor_user_id, prospect_id, request_id]);
